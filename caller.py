...
default_hyouta: str = "HyoutaToolsCLI/HyoutaToolsCLI.dll"
...
def check_dependencies():
    ...
    if not os.path.isfile(dependencies):
        ...
        with open(dependencies, 'r+') as file:
            ...
            if dependency_vesperia in data and data[dependency_vesperia]:
                vesperia = data[dependency_vesperia]

            if dependency_dotnet in data and data[dotnet]:
                dotnet = data[dependency_dotnet]

            if dependency_hyouta in data and data[dependency_hyouta]:
                hyouta = data[dependency_hyouta]

            if dependencies_comptoe in data and data[dependencies_comptoe]:
                comptoe = data[dependencies_comptoe]

            file.close()

    try:
        with open(os.path.join(vesperia, "TOV_DE.exe"), "rb") as file:
            as_bytes = file.read()
    ...
    try:
        version = subprocess.check_output([dotnet, "--version"])
    ...
    try:
        subprocess.check_output([dotnet, hyouta])
    except FileNotFoundError:
    ...
    try:
        subprocess.check_output([comptoe])
    except FileNotFoundError:
